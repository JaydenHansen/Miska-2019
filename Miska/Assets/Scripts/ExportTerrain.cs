...
class ExportTerrain : EditorWindow
{
    ...
    void Export()
    {
        ...
        m_alphaMapWidth = terrain.alphamapWidth;
        m_alphaMapHeight = terrain.alphamapHeight;

        m_splatMapData = terrain.GetAlphamaps(0, 0, m_alphaMapWidth, m_alphaMapHeight);
        ...
        if (saveFormat == SaveFormat.Triangles)
        {
            ...
            for (int y = 0; y < h - 1; y++)
            {
                for (int x = 0; x < w - 1; x++)
                {
                    if (GetMainTexture(y, x) == TerrainMaterial.Grass && GetMainTexture(y, x + 1) == TerrainMaterial.Grass && GetMainTexture(y + 1, x) == TerrainMaterial.Grass)
                    {
                    ...
                    if (GetMainTexture(y + 1, x) == TerrainMaterial.Grass && GetMainTexture(y + 1, x + 1) == TerrainMaterial.Grass && GetMainTexture(y, x + 1) == TerrainMaterial.Grass)
                    {
                        tPolys[index++] = ((y + 1) * w) + x;
    ...
    float[] GetTextureMix(int x, int y)
    {
        float[,,] splatMap = terrain.GetAlphamaps(x, y, 1, 1);
    ...
    public TerrainMaterial GetMainTexture(int x, int y)
    {
        float[] mix = GetTextureMix(x, y);
        float maxMix = 0.0f;
        int maxIndex = 0;
